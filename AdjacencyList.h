...
// 顶点的基本操作：查询第i个顶点（0 <= i < n）
 ...
 // 顶点的动态操作
    ...
    { // 插入顶点，返回编号
        for(int j = 0; j < this->n; j++) E[j].insertAsLast(nullptr); this->n++;     // 各顶点预留一条潜在的关联边
        E.insert(List<Edge<Te> *>() = nullptr);  // 创建新顶点对应的边表
...
// 边的动态操作
    ...
    { // 删除顶点i和j之间的联边（exists(i, j)）
        Te eBak = edge(i, j); delete E[i][j]; E[i][j] = nullptr; // 备仹后删除边记录
        this->e--; V[i].outDegree--; V[j].inDegree--;   // 更新边计数与关联顶点的度数
